...
│
├── 📐 Core Model
│   ├── log_attention.py            ← Custom sliding window attention module
│   ├── mini_deepseek_model.py      ← Complete causal LM (RMSNorm, SwiGLU, RoPE)
│   └── docs/deepseek_log_window.patch  ← Exact git patch for DeepSeek-V2 repo
│
├── 🏋️ Training Pipeline
│   ├── train_mini_deepseek.py      ← 5-step PyTorch training loop on log files
│   ├── save_and_upload.py          ← Verify + upload to HuggingFace Hub
│   └── pytorch_training_loop.py   ← Standalone tutorial: the 5-step loop
│
├── 🧪 Log Generation & Testing
│   ├── generate_logs.py            ← AcmeCorp payment incident log (127 lines)
│   ├── generate_3_log_files.py     ← Nginx, Kubernetes, PostgreSQL logs
...
│   └── hf_space/
│       └── index.html              ← Live HF Space (no backend needed)
│
├── 📚 Documentation
│   ├── README.md                   ← This file
...
│
└── logs/
    ├── acmecorp_payment.log        ← Generated by generate_logs.py
    ├── nginx_web_server.log        ← Generated by generate_3_log_files.py
    ├── kubernetes_pods.log
    └── postgresql_db.log
```
